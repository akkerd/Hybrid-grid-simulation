   ...
   methods      
      ...
      function SolarPanel = SolarPanel(XML, scheduleXML, currentTime, OptimParams, predictions, measures)
        ...
        %% Solar Panel
            ...
            %% Interpolations
            SolarPanel.OptimProblem.step4FreqEuro = [47.5:0.01:52.5]; % amount of values is 500
            SolarPanel.OptimProblem.interpFreqEuro = interp1(SolarPanel.Model.Params.FreqEuroCurve(1,:),...
                SolarPanel.Model.Params.FreqEuroCurve(2,:),...
            ...
            SolarPanel.OptimProblem.step4FreqPower = [0: 10 :5000];    % needs to be the same amount of values as freqPrice
...
%        result.ProductionPUproposal=SP.OptimProblem.currentSolution.ProductionPUproposal;
    ...
    function [outFreq] = getFreqFromEuro( SP, pEuro)
        % pause
%         index = find( SP.OptimProblem.interpFreqEuro( 1, :) == pEuro);
%         outFreq = SP.OptimProblem.step4FreqEuro( 2, index);
        
        index = find( SP.OptimProblem.aFreqEuroCombo( 1, :) == pEuro);
        
        outFreq = SP.OptimProblem.aFreqEuroCombo( 2, index);
    end
