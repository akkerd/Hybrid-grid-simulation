...
%        result.ProductionPUproposal=SP.OptimProblem.currentSolution.ProductionPUproposal;
    ...
    function [outFreq] = getFreqFromPrice( SP, pPrice)
        
        index = find( SP.Model.Params.FreqEuroCurve(2,:) == pPrice);
        outFreq = SP.Model.Params.FreqEuroCurve( 1, index);
        
    end
    
    function [outPower] = getPowerFromFreq( SP, pFreq )
        
        index = find( SP.Model.Params.FreqPowerCurve(1,:) == pFreq);
        outPower = SP.Model.Params.FreqPowerCurve( 2, index);
    end
