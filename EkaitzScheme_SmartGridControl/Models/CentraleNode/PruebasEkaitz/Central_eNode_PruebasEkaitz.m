...
classdef Central_eNode< eNode  
   ...
   methods      
      % Constructor
      function Central_eNode = Central_eNode(XML, scheduleXML, currentTime, OptimParams, predictions, measures)
            def=xml2struct(XML);
            
            Central_eNode.id = def.eNode.GeneralInformation.XMS_ID.Text;
          

            
            Battery.Model.Params.type = 'Central_eNode';
%           Here we read the structure of the Smart Grid
            Battery.Model.nPW=str2num(def.eNode.Model.xmlFile);  
